...
//#region node_modules/gifshot/dist/gifshot.js
var require_gifshot = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function(window, document, navigator, undefined) {
		...
		var utils = {
			...
			requestTimeout: function requestTimeout(callback, delay) {
				...
				delay = delay || 0;
				if (!utils.requestAnimFrame) return setTimeout(callback, delay);
				var start = (/* @__PURE__ */ new Date()).getTime();
				var handle = /* @__PURE__ */ new Object();
				var requestAnimFrame = utils.requestAnimFrame;
				handle.value = requestAnimFrame(function loop() {
					(/* @__PURE__ */ new Date()).getTime() - start >= delay ? callback.call() : handle.value = requestAnimFrame(loop);
				});
				return handle;
			},
			...
			each: function each(collection, callback) {
				...
				if (utils.isArray(collection)) {
					...
					while (++x < len) if (callback(x, collection[x]) === false) break;
				} else if (utils.isObject(collection)) {
					for (x in collection) if (collection.hasOwnProperty(x)) {
						if (callback(x, collection[x]) === false) break;
					}
				}
			},
			mergeOptions: function mergeOptions(defaultOptions, userOptions) {
				if (!utils.isObject(defaultOptions) || !utils.isObject(userOptions) || !Object.keys) return;
				var newObj = {};
				...
				utils.each(userOptions, function(key, val) {
					...
					if (!utils.isObject(currentUserOption)) newObj[key] = currentUserOption;
					else if (!defaultOptions[key]) newObj[key] = currentUserOption;
					else newObj[key] = utils.mergeOptions(defaultOptions[key], currentUserOption);
				});
				return newObj;
			},
			setCSSAttr: function setCSSAttr(elem, attr, val) {
				if (!utils.isElement(elem)) return;
				if (utils.isString(attr) && utils.isString(val)) elem.style[attr] = val;
				else if (utils.isObject(attr)) utils.each(attr, function(key, val) {
					elem.style[key] = val;
				});
			},
			...
			createWebWorker: function createWebWorker(content) {
				if (!utils.isString(content)) return {};
				try {
			...
			getFontSize: function getFontSize() {
				...
				if (!document.body || options.resizeFont === false) return options.fontSize;
				...
				while (span.offsetWidth > containerWidth && fontSize >= minFontSize) span.style.fontSize = --fontSize + "px";
				document.body.removeChild(span);
				return fontSize + "px";
			},
			webWorkerError: false
		};
		var utils$2 = Object.freeze({ default: utils });
		var error = {
			...
			isValid: function isValid(skipObj) {
				return error.validate(skipObj).error !== true ? true : false;
			},
			validators: [
				{
		...
		var error$2 = Object.freeze({ default: error });
		...
		var defaultOptions$2 = Object.freeze({ default: defaultOptions });
		...
		function isExistingVideoGIFSupported(codecs) {
			...
			if (utils.isArray(codecs) && codecs.length) {
				utils.each(codecs, function(indece, currentCodec) {
					if (utils.isSupported.videoCodecs[currentCodec]) hasValidCodec = true;
				});
				if (!hasValidCodec) return false;
			} else if (utils.isString(codecs) && codecs.length) {
				if (!utils.isSupported.videoCodecs[codecs]) return false;
			}
			return error.isValid({ "getUserMedia": true });
		}
		function NeuQuant() {
			...
			var initradius = initrad * (1 << radiusbiasshift);
			var radiusdec = 30;
		...
		AnimatedGIF.prototype = {
			...
			byteMap: function() {
				var byteMap = [];
				for (var i = 0; i < 256; i++) byteMap[i] = String.fromCharCode(i);
				return byteMap;
			}(),
			bufferToString: function bufferToString(buffer) {
				...
				var str = "";
				var x = -1;
				while (++x < numberValues) str += this.byteMap[buffer[x]];
				return str;
			},
			onFrameFinished: function onFrameFinished(progressCallback) {
				var self = this;
				var frames = self.frames;
				var hasExistingImages = !!(self.options.images || []).length;
				var allDone = frames.every(function(frame) {
					return !frame.beingProcessed && frame.done;
				});
				self.numRenderedFrames++;
				if (hasExistingImages) progressCallback(self.numRenderedFrames / frames.length);
				self.onRenderProgressCallback(self.numRenderedFrames * .75 / frames.length);
				if (allDone) {
					if (!self.generatingGIF) self.generateGIF(frames, self.onRenderCompleteCallback);
				} else utils.requestTimeout(function() {
			...
			processFrame: function processFrame(position) {
				...
				} else done({ "data": AnimatedGifContext.workerMethods.run(frame) });
			},
			startRendering: function startRendering(completeCallback) {
				this.onRenderCompleteCallback = completeCallback;
				for (var i = 0; i < this.options.numWorkers && i < this.frames.length; i++) this.processFrame(i);
			},
			processNextFrame: function processNextFrame() {
				...
				if (position >= 0) this.processFrame(position);
			},
			generateGIF: function generateGIF(frames, callback) {
				var buffer = [];
				var gifOptions = { loop: this.repeat };
				...
				utils.each(frames, function(iterator, frame) {
					...
					onRenderProgressCallback(.75 + .25 * frame.position * 1 / frames.length);
					for (var i = 0; i < frameDuration; i++) gifWriter$$1.addFrame(0, 0, width, height, frame.pixels, {
			...
			addFrame: function addFrame(element, gifshotOptions) {
				...
				var width = options.gifWidth;
				var height = options.gifHeight;
				var fontSize = utils.getFontSize(gifshotOptions), _gifshotOptions = gifshotOptions, filter = _gifshotOptions.filter, fontColor = _gifshotOptions.fontColor, fontFamily = _gifshotOptions.fontFamily, fontWeight = _gifshotOptions.fontWeight;
			...
			getBase64GIF: function getBase64GIF(completeCallback) {
				var self = this;
				self.startRendering(function onRenderComplete(gif) {
					self.destroyWorkers();
		...
		function existingImages() {
			...
			var ag = void 0;
			if (errorObj.error) return callback(errorObj);
			ag = new AnimatedGIF(options);
			utils.each(images, function(index, image) {
				var currentImage = image;
				if (image.src) currentImage = currentImage.src;
				if (utils.isElement(currentImage)) {
					if (options.crossOrigin) currentImage.crossOrigin = options.crossOrigin;
					loadedImages[index] = currentImage;
					loadedImagesLength += 1;
					if (loadedImagesLength === imagesLength) addLoadedImagesToGif();
				} else if (utils.isString(currentImage)) {
					tempImage = new Image();
					if (options.crossOrigin) tempImage.crossOrigin = options.crossOrigin;
					(function(tempImage) {
						if (image.text) tempImage.text = image.text;
						...
						tempImage.onload = function(e) {
							if (image.text) loadedImages[index] = {
								img: tempImage,
								text: tempImage.text
							};
							else loadedImages[index] = tempImage;
							loadedImagesLength += 1;
							if (loadedImagesLength === imagesLength) addLoadedImagesToGif();
			...
			function addLoadedImagesToGif() {
				utils.each(loadedImages, function(index, loadedImage) {
					if (loadedImage) {
						if (loadedImage.text) ag.addFrame(loadedImage.img, options, loadedImage.text);
						else ag.addFrame(loadedImage, options);
					}
				});
		...
		var screenShot = {
			getGIF: function getGIF() {
				...
				var context = void 0;
				var hasExistingImages = !!options.images.length, cameraStream = options.cameraStream, crop = options.crop, filter = options.filter, fontColor = options.fontColor, fontFamily = options.fontFamily, fontWeight = options.fontWeight, keepCameraOn = options.keepCameraOn;
				options.numWorkers;
				var progressCallback = options.progressCallback, saveRenderingContexts = options.saveRenderingContexts, savedRenderingContexts = options.savedRenderingContexts, text = options.text, textAlign = options.textAlign, textBaseline = options.textBaseline, videoElement = options.videoElement, videoHeight = options.videoHeight, videoWidth = options.videoWidth, webcamVideoElement = options.webcamVideoElement;
				...
				var captureFrames = function captureSingleFrame() {
					...
					function drawVideo() {
						try {
							if (sourceWidth > videoWidth) sourceWidth = videoWidth;
							if (sourceHeight > videoHeight) sourceHeight = videoHeight;
							...
							if (e.name === "NS_ERROR_NOT_AVAILABLE") utils.requestTimeout(drawVideo, 100);
							else throw e;
						}
					}
					function finishCapture() {
						var imageData = void 0;
						if (saveRenderingContexts) renderingContextsToSave.push(context.getImageData(0, 0, gifWidth, gifHeight));
						if (text) {
							context.font = font;
						...
						if (framesLeft > 0) utils.requestTimeout(captureSingleFrame, waitBetweenFrames);
						if (!pendingFrames) ag.getBase64GIF(function(image) {
							callback({
								"error": false,
								"errorCode": "",
								"errorMsg": "",
		...
		var videoStream = {
			...
			stream: function stream(obj) {
				...
				if (utils.isFunction(streamedCallback)) streamedCallback();
				if (existingVideo) {
				...
				} else if (videoElement.mozSrcObject) videoElement.mozSrcObject = cameraStream;
				else if (utils.URL) try {
				...
				utils.requestTimeout(function checkLoadedData() {
					...
					if (videoStream.loadedData === true) {
						...
						if (checkLoadedData.count > 10) videoStream.findVideoSize({
							videoElement,
							cameraStream,
							completedCallback
						});
						else checkLoadedData();
					}
				}, 0);
			},
			startStreaming: function startStreaming(obj) {
				...
				if (crossOrigin) videoElement.crossOrigin = options.crossOrigin;
				videoElement.autoplay = true;
				videoElement.loop = true;
				videoElement.muted = true;
				videoElement.addEventListener("loadeddata", function(event) {
					videoStream.loadedData = true;
					if (options.offset) videoElement.currentTime = options.offset;
				});
				if (existingVideo) videoStream.stream({
					videoElement,
					existingVideo,
					completedCallback
				});
				else if (lastCameraStream) videoStream.stream({
					videoElement,
					cameraStream: lastCameraStream,
					streamedCallback,
					completedCallback
				});
				else utils.getUserMedia({ video: true }, function(stream) {
					videoStream.stream({
						videoElement,
						cameraStream: stream,
						streamedCallback,
			...
			startVideoStreaming: function startVideoStreaming(callback) {
				...
				if (timeoutLength > 0) noGetUserMediaSupportTimeout = utils.requestTimeout(function() {
			...
			stopVideoStreaming: function stopVideoStreaming(obj) {
				...
				if (!keepCameraOn && hasCameraStreamTracks) {
					if (utils.isFunction(firstCameraStreamTrack.stop)) firstCameraStreamTrack.stop();
				}
				if (utils.isElement(videoElement) && !webcamVideoElement) {
					videoElement.pause();
					if (utils.isFunction(utils.URL.revokeObjectURL) && !utils.webWorkerError) {
						if (videoElement.src) utils.URL.revokeObjectURL(videoElement.src);
					}
		...
		function createAndGetGIF(obj, callback) {
			...
			if (!utils.isElement(videoElement)) return;
			...
			screenShot.getGIF(options, function(obj) {
				if ((!images || !images.length) && (!video || !video.length)) stopVideoStreaming(obj);
		...
		function existingVideo() {
			...
			if (utils.isElement(existingVideo) && existingVideo.src) {
				...
				if (!utils.isSupported.videoCodecs[videoType]) return callback(error.messages.videoCodecs);
			} else if (utils.isArray(existingVideo)) utils.each(existingVideo, function(iterator, videoSrc) {
				if (videoSrc instanceof Blob) videoType = videoSrc.type.substr(videoSrc.type.lastIndexOf("/") + 1, videoSrc.length);
				else videoType = videoSrc.substr(videoSrc.lastIndexOf(".") + 1, videoSrc.length);
		...
		function existingWebcam() {
			...
			if (!isWebCamGIFSupported()) return callback(error.validate());
		...
		function createGIF(userOptions, callback) {
			...
			if (imagesLength) existingImages({
				"images": images,
				"imagesLength": imagesLength,
				"callback": callback,
				"options": options
			});
			else if (video) existingVideo({
				"existingVideo": video,
				callback,
				options
			});
		...
		function takeSnapShot(userOptions, callback) {
			...
			createGIF(utils.mergeOptions(mergedOptions, {
				"interval": .1,
				"numFrames": 1,
