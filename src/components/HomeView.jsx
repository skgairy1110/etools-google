...
import { QrCode, Type, Image, Code, Sparkles, Layers } from 'lucide-react';

// Premium Awwwards-style color palettes for the tools
const toolThemes = [
  {
...
export default function HomeView({ onViewChange, showToast }) {
  const [activeCategory, setActiveCategory] = useState("All");
  const categories = ["All", "Generators", "Text Tools", "Media", "Developer"];

  const dummyTools = [
    { title: "QR Code Generator", desc: "Generate tracking-ready QR matrices instantly. Customize colors, logos, and error resilience.", cat: "Generators", active: true, view: 'qr', icon: QrCode },
    { title: "Text Case Converter", desc: "Instantly format your typography architecture. Switch between camel, snake, pascal, and more.", cat: "Text Tools", active: true, view: 'text-case', icon: Type },
    // UPDATE THIS LINE TO active: true 👇
    { title: "Image Compressor", desc: "Reduce payload sizes without losing visual fidelity. Smart compression algorithms.", cat: "Media", active: true, view: 'img-comp', icon: Image },
    { title: "JSON Formatter", desc: "Beautify, validate, and parse complex JSON data architectures in real-time.", cat: "Developer", active: false, view: 'json-fmt', icon: Code },
    { title: "CSS Shadow Builder", desc: "Craft perfect, buttery-smooth CSS box shadows with an intuitive visual editor.", cat: "Developer", active: false, view: 'css-shadow', icon: Layers },
    { title: "Gradient Generator", desc: "Mix and export beautiful linear and radial CSS gradients for your next project.", cat: "Generators", active: false, view: 'gradient-gen', icon: Sparkles },
  ];
  ...
  return (
    // Tightened top and bottom padding (pt-16 -> pt-8, pb-24 -> pb-12)
    <div className="w-full px-6 sm:px-12 lg:px-20 pt-8 pb-12 max-w-[1600px] mx-auto">
      
      {/* Hero Section */}
      <div className="flex flex-col items-center text-center mb-12 animate-fade-in-up"> {/* Tightened margin mb-20 -> mb-12 */}
        ...
        {/* Scaled down heading slightly to save vertical space */}
      ...
      <div className="flex flex-wrap justify-center gap-2 sm:gap-3 mb-8 animate-fade-in-up delay-100 relative z-20"> {/* Tightened margin mb-14 -> mb-8 */}
        {categories.map((cat) => (
          <button
            key={cat}
      ...
      {/* Tools Grid - Reduced card heights and padding slightly */}
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 animate-fade-in-up delay-200">
        {filteredTools.map((tool, idx) => {
          ...
          return (
            <div 
              key={idx}
              onClick={() => tool.active ? onViewChange(tool.view) : showToast(`${tool.title} is coming soon!`)}
              className={`group relative flex flex-col justify-between bg-[#050505]/60 backdrop-blur-3xl rounded-3xl p-6 sm:p-7 border border-white/[0.05] transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] cursor-pointer min-h-[220px] overflow-hidden
                ${tool.active ? `${theme.bgHover} ${theme.borderHover} ${theme.cardGlow} hover:-translate-y-2` : 'opacity-40 hover:opacity-80'}
              `}
            >
              {/* Expanding Gradient Top Line */}
              {tool.active && (
              ...
              <div className="flex justify-between items-start mb-4">
                {/* Glowing Floating Icon - Scaled down slightly */}
                ...
                {/* Status Badge */}
                {!tool.active && (
                  <span className="bg-white/5 border border-white/10 text-gray-400 text-[9px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-full">
                    Coming Soon
                  </span>
                )}
              </div>
              ...
              <div className="mt-auto">
                <div className="flex items-center gap-3 mb-2">
                  <h3 className="text-white font-extrabold text-lg sm:text-xl tracking-tight group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-white group-hover:to-gray-500 transition-all">
                    {tool.title}
                  </h3>
                </div>
              ...
              <div className="absolute -bottom-10 -right-10 opacity-0 group-hover:opacity-5 transition-opacity duration-500 pointer-events-none transform group-hover:scale-150 group-hover:-rotate-12">
                {IconComponent && <IconComponent className="w-32 h-32 text-white" strokeWidth={1} />}
              </div>
            </div>
          )
        })}
